...
<%@ include file="header.jsp"%>
   ...
   <section class="news_group">
      ...
      <c:forEach var="list" items="${pList }">
         <li>
            <a href="portfolio.do?bno=${list.bno }">
               <img src="/upload/${list.imgurl }" alt="식품경진대회 우수상 사진" class="w100">
                  ...
                  <c:forEach var="list" items="${nList}" varStatus="status">
                     <c:choose>
                        <c:when test="${status.first}">
                           <li class="recently">
                              <p class="title">
                                 <a class="title" href="noticeview.do?bno=${list.bno }&pageNum=1&amount=10">${list.title}</a>
                                 <span class="date">${list.regdate}</span>
                              </p>
                              <p class="text">첨부파일을 받아서 레이아웃 구성을 새로 하던지 연구해야 한다</p>
                           </li>
                        </c:when>
                        <c:otherwise>
                           <li><a href="noticeview.do?bno=${list.bno }&pageNum=1&amount=10">${list.title}</a><span class="date">${list.regdate}</span></li>
                        </c:otherwise>
                     </c:choose>
                  </c:forEach>
               </ul>
