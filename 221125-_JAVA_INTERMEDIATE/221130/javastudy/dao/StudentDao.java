...
public class StudentDao { // DB와 연동해서 입력(C), 검색(R), 수정(U), 삭제(D)를 함

    /* 싱글톤 객체 */
    private static StudentDao studentJdbcDao = new StudentDao();
    ...
    public void read(Student stu) {
        System.out.printf("%d\t%s\t%s\t%s\t%d\t%d\t%d\t%d\t%5.2f\t%s\n", stu.getNum(),
                stu.getName(), stu.getAddress(), stu.getTel(), stu.getKor(), stu.getEng(),
                stu.getMat(), stu.getTot(), stu.getAvg(), stu.getResult());
    }
    ...
    public int update(Student stu) {
        ...
        String query =
                "UPDATE STUDENT SET KOR = ?, ENG = ?, MAT = ?, TOT = ?, AVG = ?, RESULT = ? WHERE NUM = ?";
        ...
        try {
            ...
            pstmt.setInt(4, stu.getTot());
            pstmt.setDouble(5, stu.getAvg());
            pstmt.setString(6, stu.getResult());
            pstmt.setInt(7, stu.getNum());
